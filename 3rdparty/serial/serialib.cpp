...
/*!
     ...
     \return    -1 error while writting data
  */
int serialib::writeString(std::string receivedString)
{
#if defined (_WIN32) || defined( _WIN64)
    // Number of bytes written
    DWORD dwBytesWritten;
    ...
    int Lenght=receivedString.size();
    // Write the string
    if (write(fd,receivedString.c_str(),Lenght)!=Lenght) return -1;
    // Write operation successfull
    return 1;
#endif
}
...
{
    // receivedString.resize(maxNbBytes+1);
